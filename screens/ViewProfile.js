...
export default function ViewProfile({ route }) {
  ...
  const fetchUser = async () => {
    try {
      const response = await fetch(
        `http://10.159.139.81:8081/api/auth/user/${id}`
      );
      if (response.ok) {
  ...
  const bet_url = `http://10.159.139.81:8081/api/auth/get-bets/${id}`;
...
const Body = ({ id, bets, handleRefresh }) => {
  ...
  const BetCardBody = ({ name, bet_id }) => {
    return (
      ...
      >
        ...
        <BetCardSeeDetails id={id} bet_id={bet_id} />
      </View>
    );
  };
  ...
  const BetCardSeeDetails = ({ id, bet_id }) => {
    const [logged_in_id, setLoggedInId] = useState("");
    ...
    console.log("BetID: " + bet_id);
    console.log("Logged in user id: " + logged_in_id);
    console.log("Join bet user id: " + id);

    const betJoinBody = {
      joining_user_id: logged_in_id,
      bet_id: bet_id,
    };

    const joinBet = async (e) => {
      console.log("ID: " + logged_in_id);

      const jsonBetData = JSON.stringify(betJoinBody);

      try {
        const response = await fetch(
          "http://10.159.139.81:8081/api/auth/joinBets",
          {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
          throw new Error("Failed to join bet - check account balance");
        } else {
